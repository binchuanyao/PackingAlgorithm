    ...
    for i in range(len(skuList)):
        ...
        for choose_face in face:
            ...
            for point in O_items:
                ...
                # 如果放置点放置货物后，长不超过限制，宽+高 不超过箱体限制, 则认为可以堆放    '''软性包装袋， 对比货物 长边 、 宽+高 不超过 包装袋 长边、宽边 即可'''
                elif point[0]+curr_sku[0]<=ctnSize[0] and point[1]+curr_sku[1] + point[2]+curr_sku[2]<=ctnSize[1] + ctnSize[2]:
                    #使用放置点，添加一个图显信息
...
def packing3D(packed_result_list, show_num, O, C, Box_list, used_point):
    ...
    for i in range(0,len(Box_list)):
        ...
        if i+1 <= len(O_items):
            #如果放置点放置货物后，长不超过限制，宽+高 不超过箱体限制, 则认为可以堆放    '''软性包装袋， 对比货物 长边 、 宽+高 不超过 包装袋 长边、宽边 即可'''
            if O_items[i-1][0]+Box_list[i][0]<=C[0] and O_items[i-1][1]+Box_list[i][1] + O_items[i-1][2]+Box_list[i][2]<=C[1] + C[2]:
                #使用放置点，添加一个图显信息
            ...
            #如果轮到的这个放置点不可用
                ...
                if i+1 <= len(O_items):# and len(O_items)-1>=0:
                    #当可用点一直不可用时
                    while O_items[i-1][0]+Box_list[i][0]>C[0] or O_items[i-1][1]+Box_list[i][1] + O_items[i-1][2]+Box_list[i][2]>C[1] + C[2]:
                        #一直把可用点弹出弃用
                        O_pop.append(O_items.pop(i-1))
